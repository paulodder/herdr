...
インテグレーションはワークスペースのターミナルランチャーを設定し、今後作成される通常のタブや分割でデフォルトシェルを置き換えられます。Herdr は argv を直接実行し、`HERDR_LAUNCH_PROTOCOL=1`、`HERDR_LAUNCH_KIND=tab|split`、および分割元・方向・比率・要求された cwd のコンテキストを `HERDR_LAUNCH_*` 変数で渡します。エージェント、プラグイン、明示的なレイアウトコマンドには影響しません。ランチャーをクリアすると通常のシェル作成に戻ります。
