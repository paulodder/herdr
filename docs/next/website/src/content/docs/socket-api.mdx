...
| Area | Methods |
| --- | --- |
| Server | `ping`, `server.stop`, `server.spawn_detached`, `server.reload_config`, `server.agent_manifests`, `server.reload_agent_manifests` |
| Notification | `notification.show` |
| Client | `client.window_title.set`, `client.window_title.clear` |
| Session | `session.snapshot`, `session.watch` |
| Workspace | `workspace.create`, `workspace.list`, `workspace.get`, `workspace.focus`, `workspace.rename`, `workspace.move`, `workspace.set_terminal_launcher`, `workspace.report_metadata`, `workspace.close` |
...
stable server/session identity, the current event cursor,
...
Federation clients should use `session.watch`. With no cursor, it atomically
returns a snapshot and the cursor represented by that snapshot, then emits
newline-delimited events with monotonically increasing `cursor` values. On
reconnect, pass the last applied value as `after_cursor`; the server replays all
retained events without another snapshot. If the cursor is older than the
bounded replay window, the acknowledgement contains a replacement snapshot.
This closes the snapshot/subscribe race without requiring clients to guess
whether an event was lost.

```json
{"id":"watch","method":"session.watch","params":{"after_cursor":42}}
```

The initial response has `type: "session_watch_started"`, `cursor`, `resumed`,
and an optional `snapshot`. Each following line has `cursor`, `event`, and
`data`. Resource IDs are only server-local; federation layers must qualify them
with the endpoint plus the snapshot's `identity.server_id` and
`identity.session_id`.
