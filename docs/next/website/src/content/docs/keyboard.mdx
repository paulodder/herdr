...
## Emacs TEXT mode search

With the optional `[emacs] enabled = true` layer, press `ctrl+x [` to enter the read-only TEXT mode over a pane's full scrollback. `ctrl+s` and `ctrl+r` start forward or backward incremental search; they can also be used directly from a live pane, which enters TEXT mode automatically. Typing updates the matches and point immediately, and uppercase characters make the query case-sensitive.
