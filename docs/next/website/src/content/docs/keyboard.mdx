...
Claude Code normally uses an alternate terminal screen, so its earlier transcript may not enter Herdr's scrollback at all. Set `claude_ax_screen_reader = true` alongside `enabled = true` under `[emacs]` to make Herdr export `CLAUDE_AX_SCREEN_READER=1` in every newly created or restored pane shell. Manually typed `claude` commands and Herdr's resumed Claude sessions then inherit Claude's accessible rendering mode. After enabling or disabling the setting, create a new pane; an already-running shell cannot have its environment changed retroactively.
...
Press `ctrl+alt+o` to open the target at point. Herdr recognizes OSC 8 links, visible `http://` and `https://` URLs, existing paths relative to the pane's working directory, quoted paths, directories, and `path:line:column` references. An active TEXT-mode region is used as the exact target. URLs open in EWW; filesystem targets open in a temporary terminal `emacsclient` frame. Herdr connects to your default Emacs server and starts `emacs --daemon` when needed, so buffers survive after the temporary frame closes.
