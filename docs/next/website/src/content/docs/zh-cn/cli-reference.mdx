...
集成可以设置工作区终端启动器,用它替代今后普通标签页和分屏中的默认 shell。Herdr 直接执行 argv,并通过 `HERDR_LAUNCH_*` 变量提供 `HERDR_LAUNCH_PROTOCOL=1`、`HERDR_LAUNCH_KIND=tab|split` 以及分屏来源、方向、比例和请求 cwd 的上下文。代理、插件和显式布局命令不受影响。清除启动器即可恢复普通 shell 创建。
