...
Ctrl-click opens pane links when your terminal sends the modified click to Herdr. This works for OSC 8 hyperlinks, visible `http://` or `https://` URLs, and existing raster-image paths. A PNG opens inline when [Kitty graphics](/docs/configuration/#kitty-graphics) are enabled; otherwise Herdr opens a private local copy in your image viewer. This also works when the path belongs to another federation member. On macOS, use Ctrl-click for Herdr-handled pane links while mouse capture is enabled; Cmd-click is only available through the terminal-native bypass path, such as Shift-Cmd-click or `ui.mouse_capture = false`.
