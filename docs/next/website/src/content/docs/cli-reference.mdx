...
herdr server spawn-detached [--cwd PATH] --output PATH -- <argv...>
...
herdr workspace set-terminal-launcher <workspace_id> -- <argv...>
herdr workspace clear-terminal-launcher <workspace_id>
...
Integrations can set a workspace terminal launcher to replace the default shell
for future ordinary tabs and splits. Herdr executes argv directly and supplies
`HERDR_LAUNCH_PROTOCOL=1`, `HERDR_LAUNCH_KIND=tab|split`, and split source,
direction, ratio, and requested-cwd context through `HERDR_LAUNCH_*` variables.
Agent, plugin, and explicit layout commands are unaffected. Clear the launcher
to restore normal shell creation.
...
herdr pane set-restore-command <pane_id> -- <argv...>
herdr pane clear-restore-command <pane_id>
