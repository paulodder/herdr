...
Configure persistent Herdr sessions that should appear alongside the local
session in the navigator:
...
Each endpoint is authoritative for its own PTYs and persistent session. The
home server keeps a reconnecting `session.watch` stream over SSH and projects
qualified remote workspace, tab, pane, and agent references into the
navigator. Selecting a remote row makes the foreground client attach to that
remote Herdr natively; detaching starts a fresh local client.

Endpoint IDs must be unique and contain only letters, numbers, `.`, `_`, or
`-`. `target` is an OpenSSH destination. Background federation connections use
batch authentication, so load passphrase-protected keys into `ssh-agent`.

Use `herdr federation list`, `herdr federation watch`, and
`herdr federation attach <endpoint>` to inspect or exercise the same transport
outside the TUI.

Herdr keeps inactive native attachments ready for fast switching. To close
those client-owned transports without restarting Herdr or any persistent
server, choose **reset connections** from the sidebar menu or run
`M-x reset-federation-connections`. The active member and the home directory
connection stay attached. The action also cancels a pending remote switch when
the source connection is still healthy. Selecting another member then opens a
fresh connection. An attachment that times out is discarded automatically, so
retrying also opens a fresh transport instead of reusing the timed-out one.
...
With this enabled, Ctrl-clicking an existing PNG path shows the image inline over the active pane. Press `Esc` or `q` to close the preview; other input closes it and continues to the pane. The same gesture works across a federation: the member that owns the file validates it and sends it only to the foreground client, so the client never needs that member's filesystem mounted.

JPEG, GIF, WebP, and BMP previews open in the client's native image viewer. PNG uses the same viewer when Kitty graphics are disabled or the terminal is too small for an inline preview. Preview transfers are limited to 16 MiB, and native-viewer copies are stored in a private, short-lived local cache.
