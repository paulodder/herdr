...
pub fn run_endpoint_watch(
    ...
    loop {
        ...
        match EndpointWatch::connect(endpoint.clone(), cursor) {
            Ok(mut watch) => {
...
impl EndpointWatch {
    pub fn connect(
        ...
        let mut child = ssh_bridge_command(&endpoint)
            .stdin(Stdio::piped())
            .stdout(Stdio::piped())
            .stderr(Stdio::piped())
            .spawn()
        ...
        let request = Request {
            ...
            method: Method::SessionWatch(SessionWatchParams { after_cursor }),
        };
        ...
        let state = EndpointState {
            ...
            snapshot: snapshot.map(|snapshot| *snapshot),
            cursor: Some(cursor),
            error: None,
        };
        Ok(Self {
